...
home_winners = home_wins.merge(home_df[['match_id', 'full_name']], on='match_id')
away_winners = away_wins.merge(away_df[['match_id', 'full_name']], on='match_id')
...
if not win_counts.empty:
    ...
    print(f"🏆 Player with the most unique match wins:\nName: {top_player['full_name']}\nWins: {top_player['win_count']}")
else:
    print("⚠️ No valid winners found. Please check the winner_code values and player data.")
